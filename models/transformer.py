...
class EncoderLayer(nn.Module):
    def __init__(self, n_heads, inner_transformer_size, inner_ff_size, dropout=0.1, model_type="BERT"):
        super().__init__()
        self.mha = MultiHeadAttention(n_heads, inner_transformer_size, dropout, model_type="BERT")
        self.ff = FeedForward(inner_transformer_size, inner_ff_size, dropout, model_type="BERT")
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, out_dim=None,block_size=None, head_size=None,num_embed=None,dropout=0.1, model_type = None):
        super().__init__()
...
class FeedForward(nn.Module):
    ...
    def __init__(self, inp_dim=None, inner_dim=None,num_embed=None, dropout=0.1,model_type="BERT"):
        super().__init__()
        self.model_type = model_type
        if self.model_type == "GPT":
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, num_heads, block_size, num_embed, dropout):
        ...
        self.ffwd = FeedForward(num_embed=num_embed, dropout=dropout, model_type="GPT")
    ...
    def __init__(self, 
        ...
        if self.model_type == "BERT":
            ...
            for i in range(n_code):
                encoders += [EncoderLayer(n_heads, embed_size, inner_ff_size, dropout, model_type="BERT")]
            ...
            assert img_size % patch_size == 0, f"Image size must be divisible by patch size, image size: {img_size}, patch size: {block_size}."
